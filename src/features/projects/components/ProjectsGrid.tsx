...
import { FolderKanban, Calendar, Edit2, Building2 } from "lucide-react";
...
export function ProjectsGrid({ projects, isLoading, onEdit }: ProjectsGridProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {projects.map((p) => {
        const departmentsList = p.departments || [];
        const isOrgWide = departmentsList.length === 0;
              ...
              <div className="mt-3 flex flex-wrap items-center gap-1.5">
                ...
                {isOrgWide ? (
                  ...
                  departmentsList.map((d) => (
                    <span
                      key={d.id}
                      className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20"
                    >
                      {d.name}
                    </span>
                  ))
